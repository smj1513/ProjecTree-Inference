...
## 프로젝트 구조
```plaintext
📦 ProjecTree-Inference
├── 📂 app
│   ├── 📂 agents                 # LangGraph 기반 에이전트 모듈
│   │   ├── 📂 node               # 노드 생성 에이전트 (Epic/Story/Task)
│   │   │   ├── 📂 nodes          # 실행 노드 (Fetch, Process, Creation)
│   │   │   ├── 📜 graph.py       # 노드 생성 그래프 정의
│   │   │   └── 📜 state.py
│   │   ├── 📂 recommend          # 기술 스택 추천 에이전트
│   │   │   ├── 📂 nodes          # 전문가 노드 (Frontend/Backend/Advance Expert)
│   │   │   ├── 📜 graph.py       # 추천 그래프 정의
│   │   │   └── 📜 state.py
│   │   ├── 📂 portfolio          # 포트폴리오 생성 에이전트
│   │   │   ├── 📂 nodes          # 필터링 및 생성 노드
│   │   │   └── 📜 graph.py
│   │   └── 📂 candidates         # 후보 노드 생성 에이전트
│   ├── 📂 core                   # 핵심 공통 모듈
│   │   ├── 📜 callback.py        # Custom Async Callback Handler (Real-time Feedback)
│   │   ├── 📜 crdt_client.py     # CRDT 서버 연동 클라이언트
│   │   └── 📜 node_messages.py   # 노드별 상태 메시지 설정
│   ├── 📂 services               # 비즈니스 로직 계층
