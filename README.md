...
**해결 과정**
Langfuse를 도입하여 에이전트의 모든 실행 과정을 시각화하고 추적할 수 있는 MLOps 환경을 구축했습니다. 개발 단계에서는 복잡한 에이전트의 실행 경로(Trace)와 토큰 사용량을 분석하여 최적화 포인트를 찾아내고, 운영 단계에서는 사용자용 실시간 알림과 개발자용 상세 로그를 분리하여 모니터링 효율성을 높였습니다.
...
**문제 상황**
초기에는 자기 참조(Self-Referencing) 방식을 사용했으나, 데이터가 약 5,500만 건으로 늘어나고 계층이 깊어질수록(5단계) 재귀 쿼리의 부하가 심해졌습니다. 특정 프로젝트의 하위 트리를 모두 조회하는 데 **약 12초(12,070ms)**가 소요되어 실시간 칸반 보드 렌더링이 불가능했습니다.
